...
# them phan xu li ban bong bay vao muc tieu
#Thêm teleport
...
class RAINBULLET:
    def __init__(self):
        self.bullet_all = [Vector2(8,1),Vector2(1,1), Vector2(1,5)]
        self.many_bl = [Vector2(1,1), Vector2(1,5)]
        #self.n = 2
        self.many_relations = [Vector2(1,0), Vector2(0,1)]
...
class MAIN:
        ...
        def check_possition(self):
            ...
            if self.mode == "attack" and self.isShot2:
                #check bóng chạm mục tiêu thì được điểm
                if self.bullet.pos == self.fruit.pos:
                    self.fruit.randomize()
                    self.snake.add_block()
                    self.snake.play_crunch_sound()
                    self.isShot2 = False
                    self.bullet.isShot = True
                    

            # if self.mode == "attack":
            #     # check bóng chạm tường thì thiết lập lại vị trí bóng
            #     if 0 > self.bullet.pos.x or self.bullet.pos.x > cell_number_width or  0 > self.bullet.pos.y or self.bullet.pos.y > cell_number_height:
            #         self.bullet.shoted()
            #         self.isShot2 = False
            #         self.bullet.isShot = True
                ...
                for i in range(len(self.many_bullets.many_bl)):
                    for j in range(self.fence.num_fence):
                        if self.many_bullets.many_bl[i] == self.fence.location[j]:
                            self.snake.add_block()
                            self.snake.play_crunch_sound()
                            #del self.many_bullets.many_bl[i]
                            #del self.many_bullets.many_relations[i]
                            del self.fence.location[j]
                            self.fence.num_fence -= 1
                            break
        ...
        def check_fail(self):
            ...
            if (self.mode == "attack" and self.isShot2) or self.mode == "rain":
                for block in self.snake.body[1:]:
                    if block == self.bullet.pos:
                        overGame()
                
            #đầu rắn chạm vào vật mà không bắn bóng thì thua
            if (self.mode == "attack" and self.isShot2 == False) or self.mode == "rain":
                if self.fruit.pos == self.snake.body[0]:
                    overGame()

        def draw_snake_on_head(self):
            #xu li ran di xuyen tuong
...
def overGame():
    ...
    while True:
        ...
        SCORE_RECT = SCORE_TEXT.get_rect(center=(240, 360))
        screen.blit(SCORE_TEXT, SCORE_RECT)

        PLAY_BACK = Button(image=None, pos=(640, 460), 
                            text_input="BACK", font=get_font(75), base_color="White", hovering_color="Green")
        PLAY_AGAIN = Button(image=None, pos=(640, 660), 
...
def optionsGame():
    while True:
        ...
        OPTIONS_RESUME = Button(image=None, pos=(640, 260), 
                            text_input="RESUME", font=get_font(75), base_color="Black", hovering_color="Green")
        OPTIONS_MUSIC = Button(image=None, pos=(640, 400), 
                            text_input="MUSIC", font=get_font(75), base_color="Black", hovering_color="Green")
...
def modeGame():
    while True:
        ...
        OPTIONS_TEXT = get_font(45).render("This is the New Game screen.", True, "Black")
        OPTIONS_RECT = OPTIONS_TEXT.get_rect(center=(640, 260))
        screen.blit(OPTIONS_TEXT, OPTIONS_RECT)

        OPTIONS_CLASSIC = Button(image=None, pos=(640, 260), 
                            text_input="CLASSIC", font=get_font(75), base_color="Black", hovering_color="Green")
        OPTIONS_TROUBLE = Button(image=None, pos=(640, 400), 
                            text_input="TROUBLE", font=get_font(75), base_color="Black", hovering_color="Green")
        OPTIONS_ATTACK = Button(image=None, pos=(640, 560), 
                            text_input="ATTACK", font=get_font(75), base_color="Black", hovering_color="Green")
        OPTIONS_RAIN_ATTACK = Button(image=None, pos=(640, 700), 
                            text_input="RAIN ATTACK", font=get_font(75), base_color="Black", hovering_color="Green")
...
def options():
    while True:
        ...
        OPTIONS_MUSIC = Button(image=None, pos=(640, 260), 
                            text_input="MUSIC", font=get_font(75), base_color="Black", hovering_color="Green")
        OPTIONS_HELP = Button(image=None, pos=(640, 400), 
                            text_input="HELP", font=get_font(75), base_color="Black", hovering_color="Green")
        OPTIONS_BACK = Button(image=None, pos=(640, 560), 
                            text_input="BACK", font=get_font(75), base_color="Black", hovering_color="Green")
...
def help():
    while True:
        ...
        OPTIONS_RECT = OPTIONS_TEXT.get_rect(center=(100, 60))
        
        screen.blit(OPTIONS_TEXT, OPTIONS_RECT)

        texts = ["1.aaaaaaaaaaaaaaaaaaaa", "2.bbbbbbbbbbbbbbbbbbbbbbbb", "3. cccccccccccccccccccccccc"]
        line = 80
        for text in texts:
            OPTIONS_TEXT = get_font(15).render(text, True, "Black")
            OPTIONS_RECT = OPTIONS_TEXT.get_rect(center=(100, line))
            line+=20
            screen.blit(OPTIONS_TEXT, OPTIONS_RECT)
...
def main_menu():
    while True:
        ...
        MENU_RECT = MENU_TEXT.get_rect(center=(100, 100))
